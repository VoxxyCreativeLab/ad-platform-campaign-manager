...
Visual, social-media-style campaigns that run across YouTube (in-feed, Shorts, in-stream), Google Discover, Gmail, and the Google Display Network. Designed for mid-funnel engagement and conversion-focused video — reaching users in a "discovery" mindset with rich creative.
...
> [!info] Video Action Campaigns Absorbed
> All Video Action Campaigns were migrated to Demand Gen (creation blocked March 2025, migration completed April 2026). Demand Gen is now also Google's **conversion-focused video campaign type**, not just mid-funnel. If you previously used Video Action campaigns, you're now using Demand Gen.
...
|--------|------------|---------|-------|
| Placements | YouTube in-feed, Shorts, in-stream, Discover, Gmail, **GDN (3M+ sites/apps)** | Google Display Network (3M+ sites) | YouTube + Video Partners |
| Creative | Image, video, carousel | Responsive display ads | Video only |
| Intent level | Medium-High (discovery + conversions) | Low (passive browsing) | Low-Medium |
| Audience focus | Strong (lookalikes, customer lists) | Medium | Medium |
| Best for | Mid-funnel engagement + conversion-focused video (absorbed Video Action) | Remarketing, broad reach | Awareness, YouTube presence |
...
| Google Display Network | Browsing 3M+ websites and apps | Image ads (since April 2025) |

> [!info] GDN Expansion
> Since April 2025, Demand Gen includes image inventory across the Google Display Network (3M+ sites and apps). "Google Video Partners" within Demand Gen was renamed to "Google Display Network" to reflect this broader reach. Advertisers can choose exactly which surfaces (YouTube, GDN, Discover, Gmail) at **ad group level** using channel controls.
...
> [!tip] Lookalike Segments → Audience Suggestions
...
>
> **Transition (March 2026):** Lookalike segments are transitioning to **"audience suggestions"** powered by AI. The reach slider (narrow/balanced/broad) is changing to an AI-driven model that automatically determines expansion. Existing lookalike segments continue to work during the transition.
...
| Target CPC | More manual-like click-level control (October 2025) — good for transitioning from manual bidding |
| Maximize Conversion Value | E-commerce — optimize for revenue |
| Target ROAS | Mature campaigns with consistent ROAS data |

### Channel Controls

Advertisers can choose exactly which surfaces to include at the **ad group level**:
...
This means you can run a Demand Gen campaign that only targets YouTube Shorts + GDN, or only Discover + Gmail. Use channel controls to align ad groups with creative format strengths.

### New Features (2025-2026)

| Feature | Description |
|---------|-------------|
| AI video generation (Veo) | Create video ads from static images using Google's Veo model — useful for advertisers without video production capacity |
| Shoppable CTV | Connected TV ads on YouTube — viewers can interact with product cards on their TV screen |
| New Customer Only Mode | New customer acquisition goal — bid exclusively for users Google identifies as non-customers |
| Creator partnerships | Partner with YouTube creators for Shorts inventory — ads appear in creator content with creator attribution |

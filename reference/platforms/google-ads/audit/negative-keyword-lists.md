---
title: Negative Keyword Lists by Category
date: 2026-04-01
tags:
  - reference
  - google-ads
  - audit
---

# Negative Keyword Lists by Category

Pre-built negative keyword lists to prevent wasted spend. Add these as shared negative keyword lists at the account level, then apply to relevant campaigns.

> [!tip] PMax Support
> Since August 2025, shared negative keyword lists can be applied to Performance Max campaigns. Previously this required a Google rep — it's now self-service in the Google Ads UI.

## Universal Negatives (Apply to Most Campaigns)

### Job/Career Related
```
jobs
career
careers
hiring
salary
salaries
internship
internships
volunteer
employment
employer
resume
cv
vacancy
vacancies
recruit
recruiting
recruitment
glassdoor
indeed
```

### Information/Research Only
```
what is
how to
how do
tutorial
tutorials
guide
definition
meaning
wikipedia
example
examples
template
templates
sample
samples
course
courses
training
certification
class
classes
learn
learning
study
```

### Free/Cheap Seekers
```
free
gratis
cheap
cheapest
discount
coupon
coupons
promo code
voucher
deal
deals
budget
affordable
low cost
```

### DIY/Self-Service
```
diy
do it yourself
homemade
make your own
build your own
```

## B2B Negatives (Apply When Targeting Businesses)

```
personal
home use
residential
consumer
hobby
hobbyist
student
kids
children
```

## B2C Negatives (Apply When Targeting Consumers)

```
wholesale
bulk
enterprise
corporate
b2b
reseller
white label
api
integration
```

## E-commerce Negatives

### Returns/Complaints
```
return policy
refund
complaint
complaints
recall
recalled
lawsuit
scam
review
reviews
rating
ratings
```

### Comparison/Research
```
vs
versus
compare
comparison
alternative
alternatives
best
top 10
ranking
```

## Service Business Negatives

```
near me salary
[service] jobs
how to become a [profession]
[profession] school
[profession] degree
[profession] certification
```

## Industry-Specific Suggestions

### Legal Services
```
pro bono
legal aid
free consultation (if you charge for consultations)
law school
bar exam
```

### Healthcare
```
symptoms
diagnosis
wikipedia
nhs
webmd
home remedies
natural cure
```

### Real Estate
```
zillow
rightmove
rent (if selling only)
buy (if renting only)
for sale by owner (if you're an agent)
```

### SaaS/Software
```
open source
free alternative
crack
torrent
pirated
github
```

## Usage Instructions

1. Go to Google Ads → Tools → Shared library → Negative keyword lists
2. Create a new list for each category
3. Add the relevant keywords from above
4. Apply lists to appropriate campaigns
5. Review and expand quarterly based on search term reports
6. Don't over-negate — check if negatives are blocking valuable traffic
